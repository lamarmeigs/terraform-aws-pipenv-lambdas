...
New releases are automatically generated by a CI workflow upon merge to the `main` branch.
Their version numbers are inferred from the [CHANGELOG](CHANGELOG.md).

The expected development workflow is as follows:

1. Fork the repository.
2. Create a feature branch off the `main` branch.
3. Commit any required changes to the feature branch. Be sure to update the CHANGELOG with a new
   version (adhering to the [Keep A Changelog](https://keepachangelog.com/) standard)!
4. Create a Pull Request against the base branch.
5. Upon approval, merge.
   If any Terraform or Python files have been changed, a new version will be created as a git tag
   and GitHub Release.
